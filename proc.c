...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  ENQUEUE(&prioq[RDSL_STARTING_LEVEL], p);
...
yield(void)
{
  ...
  myproc()->set = EXPIRED;
  myproc()->level = level+1; //Move a level up
  ENQUEUE(&prioq[level+1], myproc());
  sched();
  release(&ptable.lock);
}
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  int level = p->level;
  REMOVE(&prioq[level], p);
  p->state = SLEEPING;
  ENQUEUE(&prioq[level], p);
  sched();
